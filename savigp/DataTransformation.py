...
class MeanStdYTransformation(DataTransformation):
    ...
    def untransform_NLPD(self, NLPD):
        return NLPD + np.hstack((np.array([np.log(self.scaler.var_).sum() / 2]), np.log(self.scaler.var_) / 2))

class OneHotYTransformation(DataTransformation):
    def __init__(self, X, Y, label_Num):
        """
        Y is like [[1],
                   [3], 
                   [5], 
        ...
        self.encoder.fit(np.arange(label_Num)[:, None])

    def transform_X(self, X):
        return X

    def transform_Y(self, Y):
        return self.encoder.transform(Y).toarray()

    def untransform_X(self, X):
        return X

    def untransform_Y(self, Y):
        return np.argmax(Y, axis = 1)[:, None]
...
class MeanStdTransformation(DataTransformation):
    ...
    def untransform_NLPD(self, NLPD):
        return NLPD + np.hstack([np.array([np.log(self.output_scaler.var_).sum() / 2]),
                                 np.log(self.output_scaler.var_) / 2])
